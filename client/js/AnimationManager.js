// AnimationManager.js – исправлено: прерывание анимации

window.AnimationManager = (function() {
    const imageCache = new Map();
    const activeAnimations = { hero: false, enemy: false };
    const animationTimeouts = { hero: null, enemy: null }; // <-- добавлено
    ...
    function playAnimation(target, animType, options = {}) {
        ...
        return new Promise((resolve) => {
            // ========== НОВЫЙ БЛОК: прерывание предыдущей анимации ==========
            if (activeAnimations[target]) {
                clearTimeout(animationTimeouts[target]);
            ...
            animationTimeouts[target] = setTimeout(() => {
                container.style.display = 'none';
                container.innerHTML = '';
                activeAnimations[target] = false;
                animationTimeouts[target] = null;
