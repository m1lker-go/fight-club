...
function showTowerHelp() {
    ...
    const towerDescription = `
        <div class="role-card">
            <h3>🏰 О башне</h3>
            <p>Каждый сезон вы выбираете одного героя, за которого проходите башню. Менять класс нельзя (кроме особых билетов).</p>
            <p>Каждый день даётся <strong>10 билетов</strong> на попытки. Каждая попытка (победа или поражение) тратит 1 билет. Билеты обновляются в 00:00 по Москве.</p>
            <p>За победу на этаже вы получаете монеты или редкие скины (каждые 20 этажей). С каждым этажом враги становятся сильнее.</p>
        </div>
    `;

    const mouseDescriptions = [
        {
            name: 'Мышь-некромант (этаж 5)',
            passive: 'Воскрешение – один раз за бой при смертельном уроне восстанавливает 10% HP и сбрасывает негативные эффекты.',
            active: 'Неистовство нежити – наносит три удара подряд. Урон каждого: 3–8 + 0.5 * этаж (магический).'
        },
        {
            name: 'Клинок (этаж 10)',
            passive: 'Скорость тьмы – скорость всегда 100 (ходит первым).',
            active: 'Смертельный танец – наносит два удара подряд, каждый с гарантированным критическим уроном. Урон каждого: 10 + этаж * 2.'
        },
        {
            name: 'Антимаг (этаж 15)',
            passive: 'Поглощение маны – каждая атака крадёт у цели 5 маны и восстанавливает себе 5 маны.',
            active: 'Антимагический удар – урон зависит от текущей маны цели: 0–20 маны → 15 + этаж*2; 21–70 → 10 + этаж*2; 71+ → 5 + этаж*2.'
        },
        {
            name: 'Паладин (этаж 20)',
            passive: 'Божественный щит – получает на 50% меньше урона (после расчёта защиты).',
            active: 'Неуязвимость – на 2 хода не получает никакого урона.'
        },
        {
            name: 'Алхимик (этаж 25)',
            passive: 'Ядовитая атака – каждый удар накладывает отравление на 1 ход. В конце хода цели наносится урон, равный 50% от урона атаки (округляется вверх). Эффект не стакается.',
            active: 'Адский коктейль – наносит мощный урон (20 + этаж * 3) и гарантированно отравляет цель на 2 хода с уроном 10 + этаж * 2 в конце каждого хода.'
        },
        {
            name: 'Тень (этаж 30)',
            passive: 'Призрачная тень – 90% шанс уклониться от любой атаки (не зависит от ловкости).',
            active: 'Исчезновение – на 1 ход становится невидимой (100% уклонение) и в конце этого хода наносит дополнительный урон 20 + этаж * 4, игнорирующий защиту.'
        }
    ];
    ...
    mouseDescriptions.forEach(mouse => {
        miceHtml += `
            <div class="role-card">
                <h3>${mouse.name}</h3>
                <p><span class="passive">Пассивный:</span> ${mouse.passive}</p>
                <p><span class="active">Активный:</span> ${mouse.active}</p>
            </div>
        `;
    });
