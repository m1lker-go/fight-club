...
// Возвращает дату турнира, которую нужно показывать:
// - если сейчас < 10:00 утра → показываем вчерашний турнир
// - иначе → сегодняшний
function getRelevantTournamentDate() {
...
router.get('/status', async (req, res) => {
    ...
    try {
        ...
        const todayStr = getMoscowDateString();
        const relevantDate = getRelevantTournamentDate();
        const isTodayTournament = (relevantDate === todayStr);

        // Регистрация возможна только для сегодняшнего турнира и в интервале 10:00–19:50
        let canRegister = false;
        if (isTodayTournament) {
            canRegister = (currentHour > REGISTRATION_START_HOUR || (currentHour === REGISTRATION_START_HOUR && currentMinute >= 0)) &&
                          (currentHour < REGISTRATION_DEADLINE_HOUR || (currentHour === REGISTRATION_DEADLINE_HOUR && currentMinute <= 50));
        }

        // Активен ли турнир (идет бой) – только для сегодняшнего, с 20:00 до 20:10
        let tournamentActive = false;
        if (isTodayTournament) {
            tournamentActive = (currentHour >= TOURNAMENT_START_HOUR && currentHour < TOURNAMENT_START_HOUR + 1 && currentMinute < 10);
        }

        // Проверяем, есть ли матчи для релевантной даты (т.е. завершён ли уже турнир)
        ...
        // Проверяем регистрацию пользователя на релевантную дату
...
router.get('/bracket', async (req, res) => {
    ...
    const client = await pool.connect();
    try {
        const tournamentDate = getRelevantTournamentDate();
