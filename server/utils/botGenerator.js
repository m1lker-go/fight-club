// server/utils/botGenerator.js (локализация имён ботов + предметы)

const fs = require('fs');
const path = require('path');
const { getItemName, fixedBonuses } = require('../data/itemData');
...
// ------------------ ЗАГРУЗКА ПЕРЕВОДОВ ДЛЯ БОТОВ ------------------
let cachedBotRu = null;
let cachedBotEn = null;
let botTranslationsLoaded = false;

function loadBotTranslations() {
    if (botTranslationsLoaded) return;
    try {
        ...
        if (fs.existsSync(ruPath)) {
            ...
            cachedBotRu = ruData.bot_generator || {};
        }
        if (fs.existsSync(enPath)) {
            const enData = JSON.parse(fs.readFileSync(enPath, 'utf-8'));
            cachedBotEn = enData.bot_generator || {};
        }
        botTranslationsLoaded = true;
        console.log('✅ Bot name translations loaded');
    } catch (e) {
        console.error('❌ Failed to load bot name translations:', e.message);
        botTranslationsLoaded = true;
    }
}

/**
 * Возвращает локализованное имя бота по ключу.
 * Ключи: 'wooden_dummy', 'silver_defender', 'gold_defender', 'cybercat'
 * Для мышиных боссов ключи в 'mouseBosses': 'necromancer', 'blade', 'antimag', 'paladin', 'alchemist', 'shadow'
 */
function getBotName(lang, key, fallback) {
    loadBotTranslations();
    if (lang === 'en' && cachedBotEn) {
        if (cachedBotEn.botNames && cachedBotEn.botNames[key]) return cachedBotEn.botNames[key];
        if (cachedBotEn.mouseBosses && cachedBotEn.mouseBosses[key]) return cachedBotEn.mouseBosses[key];
        if (cachedBotEn[key]) return cachedBotEn[key];
    }
    if (lang === 'ru' && cachedBotRu) {
        if (cachedBotRu.botNames && cachedBotRu.botNames[key]) return cachedBotRu.botNames[key];
        if (cachedBotRu.mouseBosses && cachedBotRu.mouseBosses[key]) return cachedBotRu.mouseBosses[key];
        if (cachedBotRu[key]) return cachedBotRu[key];
    }
    return fallback || key;
}

// Дефолтные русские имена (если перевод не найден)
const defaultBotNames = {
    wooden_dummy: 'Деревянный манекен',
    silver_defender: 'Серебряный защитник',
    gold_defender: 'Золотой защитник',
    cybercat: 'Киберкот'
};
const defaultMouseBossNames = {
    necromancer: 'Мышь-некромант',
    blade: 'Клинок',
    antimag: 'Антимаг',
    paladin: 'Паладин',
    alchemist: 'Алхимик',
...
// Шаблоны обычных ботов (имена теперь динамические)
const botTemplates = [
    { nameKey: 'wooden_dummy', class: 'warrior', subclass: 'guardian' },
    { nameKey: 'wooden_dummy', class: 'warrior', subclass: 'berserker' },
    { nameKey: 'wooden_dummy', class: 'warrior', subclass: 'knight' },
    { nameKey: 'silver_defender', class: 'assassin', subclass: 'assassin' },
    { nameKey: 'silver_defender', class: 'assassin', subclass: 'venom_blade' },
    { nameKey: 'silver_defender', class: 'assassin', subclass: 'blood_hunter' },
    { nameKey: 'gold_defender', class: 'mage', subclass: 'pyromancer' },
    { nameKey: 'gold_defender', class: 'mage', subclass: 'cryomancer' },
    { nameKey: 'gold_defender', class: 'mage', subclass: 'illusionist' }
];
...
function applyClassBonuses(stats, className, subclass) {
    ...
    result.manaRegen = Math.min(35, result.manaRegen || 0);
...
function generateBotItem(className, level, lang = 'ru') {
    let rarity = 'common';
    ...
    // Используем getItemName с языком
    const index = Math.floor(Math.random() * 5); // в каждом массиве 5 названий
    const name = getItemName(lang, className, type, rarity, index);
...
function generateNormalBot(playerLevel, forcedClass = null, forcedSubclass = null, lang = 'ru') {
    let level, className, subclass, nameKey = null;

    if (forcedClass && forcedSubclass) {
        level = Math.min(60, Math.max(1, playerLevel));
        className = forcedClass;
        subclass = forcedSubclass;
        // Без шаблона возьмём первое попавшееся имя для этого класса
        const fallbackNames = { warrior: 'wooden_dummy', assassin: 'silver_defender', mage: 'gold_defender' };
        nameKey = fallbackNames[className] || 'wooden_dummy';
    } else {
    ...
    const numItems = Math.min(4, Math.floor(level / 10) + Math.floor(Math.random() * 2));
    const botItems = [];
    for (let i = 0; i < numItems; i++) {
        const item = generateBotItem(className, level, lang);
    ...
    const username = getBotName(lang, nameKey, defaultBotNames[nameKey] || nameKey);
...
function generateCybercat(playerLevel, lang = 'ru') {
    const bot = generateNormalBot(playerLevel, undefined, undefined, lang);
    bot.username = getBotName(lang, 'cybercat', defaultBotNames.cybercat);
...
// ========== МЫШИНЫЕ БОССЫ (БАШНЯ) ==========
const mouseBosses = [
    { type: 'necromancer', nameKey: 'necromancer', avatar: 'mouse-skin-necr.png', subclass: 'mouse_necromancer',
      baseHp: 80, baseAtk: 12, baseDef: 8, baseAgi: 15, baseInt: 15, baseSpd: 18 },
    { type: 'blade', nameKey: 'blade', avatar: 'mouse-skin-blade.png', subclass: 'mouse_blade',
      baseHp: 70, baseAtk: 18, baseDef: 5, baseAgi: 25, baseInt: 5, baseSpd: 25 },
    { type: 'antimag', nameKey: 'antimag', avatar: 'mouse-skin-antimag.png', subclass: 'mouse_antimag',
      baseHp: 75, baseAtk: 14, baseDef: 6, baseAgi: 20, baseInt: 20, baseSpd: 20 },
    { type: 'paladin', nameKey: 'paladin', avatar: 'mouse-skin-titan.png', subclass: 'mouse_paladin',
      baseHp: 100, baseAtk: 10, baseDef: 12, baseAgi: 10, baseInt: 10, baseSpd: 12 },
    { type: 'alchemist', nameKey: 'alchemist', avatar: 'icon-mouse-alchim.png', subclass: 'mouse_alchemist',
      baseHp: 85, baseAtk: 13, baseDef: 7, baseAgi: 18, baseInt: 18, baseSpd: 16 },
    { type: 'shadow', nameKey: 'shadow', avatar: 'mouse-skin-shadow.png', subclass: 'mouse_shadow',
      baseHp: 65, baseAtk: 16, baseDef: 4, baseAgi: 30, baseInt: 8, baseSpd: 28 }
];

function generateMouseBoss(floor, lang = 'ru') {
    const bossIndex = (Math.floor(floor / 5) - 1) % mouseBosses.length;
    const bossTemplate = mouseBosses[bossIndex];
    const scale = 1 + (floor - 5) / 50;
    const name = getBotName(lang, bossTemplate.nameKey, defaultMouseBossNames[bossTemplate.nameKey] || bossTemplate.nameKey);
    return {
        username: name,
        avatar_id: null,
...
function generateBot(playerLevel, isCybercat = false, forcedClass = null, forcedSubclass = null, lang = 'ru') {
    if (isCybercat) return generateCybercat(playerLevel, lang);
    return generateNormalBot(playerLevel, forcedClass, forcedSubclass, lang);
}

// Генерация предмета по редкости (используется в адвенте)
function generateItemByRarity(rarity, ownerClass = null, lang = 'ru') {
    ...
    const name = getItemName(lang, chosenClass, type, rarity, index);
